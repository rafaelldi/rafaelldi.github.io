...
Ok, we’ve already done this in a [previous part](/posts/asp-net-core-logs-part-1.html), but now we need a [special formatter](https://github.com/serilog/serilog-sinks-elasticsearch), which is compatible with Elasticsearch (`Serilog.Formatting.Elasticsearch`). Install this nuget and change the config file.
